...
| M6 통계와 원시 데이터 | 1.7–3.8% 차이를 "outperform"이라고 쓰던 주장을 철회. 측정 구간, 기기, 측정기는 `\authorcheck`. **분석 파이프라인을 Jupyter 노트북으로 추가**(`notebooks/uatimer_analysis.ipynb`): 원시 per-run CSV를 읽어 평균·95% 신뢰구간·오차 전파·Wilcoxon 유의성 검정·그림·LaTeX 표(Table V/VI)를 자동 생성하고, 측정용 템플릿(전력계 루프, 트레이스 재생)도 포함. 데이터를 지어내지 않도록 실측 CSV를 저자가 채우는 구조이며 데모 데이터는 난수임을 명시. **원시 데이터 자체와 재실험은 원고로 대체 불가** | 분석 인프라 완료. **저자가 실측 per-run 데이터를 2차로 제공(`data/raw_runs.csv`, 조건별 20회).** 평균±95% CI, 오차 전파 절감률, Bonferroni 보정 Wilcoxon 검정을 Table V/VI와 본문에 반영. UATimer는 Baseline·Fixed 대비 5개 워크로드 모두 유의(p<10⁻³); vs LR은 작아(1.7–3.8%) 다중비교 보정 통과 못 함 → 우수 주장 안 함. **M6 해결** |
